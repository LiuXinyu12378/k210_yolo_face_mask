...
int region_layer_init(region_layer_t *rl, int width, int height, int channels,
...
static void softmax(region_layer_t *rl, float *input, int n, int stride,
                    float *output) {
  int i;
  float diff;
  float e;
...
static void softmax_cpu(region_layer_t *rl, float *input, int n, int batch,
                        int batch_offset, int groups, int stride,
                        float *output) {
  int g, b;

  for (b = 0; b < batch; ++b) {
    for (g = 0; g < groups; ++g)
      softmax(rl, input + b * batch_offset + g, n, stride,
...
static void forward_region_layer(region_layer_t *rl) {
  ...
  softmax_cpu(rl, rl->input + index, rl->classes, rl->anchor_number,
              rl->output_number / rl->anchor_number,
...
static box_t get_region_box(float *x, float *biases, int n, int index, int i,
...
static void get_region_boxes(region_layer_t *rl, float *predictions,
                             float **probs, box_t *boxes) {
  ...
  for (int i = 0; i < layer_width * layer_height; ++i) {
    ...
    for (int n = 0; n < anchor_number; ++n) {
      ...
      int obj_index =
          entry_index(rl, n * layer_width * layer_height + i, coords);
      int box_index = entry_index(rl, n * layer_width * layer_height + i, 0);
      float scale = predictions[obj_index];

      boxes[index] =
          get_region_box(predictions, rl->anchor, n, box_index, col, row,
...
static void region_layer_output(region_layer_t *rl, obj_info_t *obj_info) {
  ...
  for (int i = 0; i < rl->boxes_number; ++i) {
    ...
    if (prob > threshold) {
      box_t *b = boxes + i;
      obj_info->obj[obj_number].x1 =
          b->x * image_width - (b->w * image_width / 2);
      obj_info->obj[obj_number].y1 =
          b->y * image_height - (b->h * image_height / 2);
      obj_info->obj[obj_number].x2 =
          b->x * image_width + (b->w * image_width / 2);
      obj_info->obj[obj_number].y2 =
          b->y * image_height + (b->h * image_height / 2);
